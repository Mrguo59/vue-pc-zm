  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="userAdd in trade.userAddressList"
        :key="userAdd.id"
      >
        <span
          :class="{ username: true, selected: userAdd.id === selectAddressId }"
          @click="selectAddressId = userAdd.id"
          >{{ userAdd.consignee }}</span
        >
        <p>
          <span class="s1">{{ userAdd.userAddress }}</span>
          <span class="s2">{{ userAdd.phoneNum }}</span>
          <span class="s3" v-if="userAdd.isDefault === '1'">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
    ...
    <div class="sub clearFix">
      <button class="subBtn" @click="placeOrder">提交订单</button>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    selectAddress() {
      ...
      //找出当前点击的是哪个地址，然后把点击的地址对象返回
      return userAddressList
        ? userAddressList.find((userAdd) => userAdd.id === selectAddressId)
        : {};
    },
  },
  async mounted() {
    ...
    this.trade = trade;
    //找出isDefault为1的,然后设置为默认选中
    this.selectAddressId = trade.userAddressList.find(
      (userAdd) => userAdd.isDefault === "1"
    ).id;
  },
};
</script>
